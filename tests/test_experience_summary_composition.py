...
def _seed(output_dir, *, ctx_extra=None):
    ...
    try:
        ...
        session.add(c)
        session.flush()
        a = Application(candidate_id=c.id, title="Senior PM",
        ...
        session.add(e)
        session.flush()
        session.add(ExperienceTitle(experience_id=e.id, title="Lead PM",
                                    is_official=1, source="official"))
        vids = []
        for i, text in enumerate(["Platform-scale framing.", "Growth-builder framing."]):
            si = ExperienceSummaryItem(experience_id=e.id, text=text,
    ...
    ctx = {"application_id": aid, "iteration": 0,
...
class TestGetComposition:
    def test_surfaces_per_role_summary_block(self, comp_app):
        ...
        ctx["llm_experience_summary_recommendations"] = {"recommendations": [
            {"experience_id": eid, "summary_item_id": vids[1],
             "rationale": "growth fits", "alternates": []},
        ]}
...
class TestPostComposition:
    ...
    def test_variant_from_other_role_rejected(self, comp_app):
        ...
        try:
            ...
            session.add(other)
            session.flush()
            foreign = ExperienceSummaryItem(experience_id=other.id, text="Foreign.",
                                            is_active=1)
            session.add(foreign)
            session.commit()
...
class TestGeneratePromptByteIdentity:
    ...
    def _ctx(self, *, with_summary):
        exp = {"id": 1, "company": "Acme", "start_date": "2021-01",
               "end_date": None, "eligible_titles": [], "bullets": [
                   {"id": 7, "text": "Shipped a platform.", "tags": [],
                    "has_outcome": True}]}
        if with_summary:
    ...
    def test_summary_present_emits_element_and_guide(self):
        ...
        assert ("<summary>Owned platform scale across teams.</summary>"
                in _stable_user_prefix(ctx))
