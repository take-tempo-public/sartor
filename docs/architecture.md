...
| [`recall/`](../recall/) | **Memory substrate** (Stage 0 skeleton): deterministic, provenance-stamped retrieval + assembly seam that *feeds* the doc-grounded avatar (7.5). Stdlib-only; refactor-immune (`tests/test_recall_boundary.py` enforces the boundary) | `Unit`, `Source`, `Scope`, `Context`, `assemble()` | LLM calls, `app.py`/`analyzer`/DB/Flask imports, real source tiers (those are 7.5/7.6) |
