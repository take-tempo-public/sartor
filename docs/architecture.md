...
| [`recall/`](../recall/) | **Memory substrate** (Stage 1): deterministic, provenance-stamped retrieval + assembly that *feeds* the doc-grounded avatar. Stdlib-only; refactor-immune (`tests/test_recall_boundary.py` enforces the boundary). `recall/sources/` adds the generic, injected `WikiSource` (S1) / `GitGrepSource` (S2) / `SessionSource` (S5-P1) tiers | `Unit`, `Source`, `Scope`, `Context`, `assemble()`, `WikiSource`, `GitGrepSource`, `SessionSource` | LLM calls, `app.py`/`analyzer`/DB/Flask imports, callback-specific paths baked into the tiers (those are injected by the wiring layer), the S3 vector tier (7.6) |
| [`blueprints/`](../blueprints/) | Flask route modules split out of the `app.py` monolith (born 7.5; the v1.0.8 split target). `assistant.py` = the doc-grounded assistant's SSE route (`POST /api/assistant/ask`) + the callback wiring (source roots + SCHEMA audience rules) binding the generic `recall.sources` tiers; the avatar LLM call itself stays in `analyzer.py` | `assistant_bp` | the LLM call (that is `analyzer.avatar_answer_streaming`); importing `app.py` |
