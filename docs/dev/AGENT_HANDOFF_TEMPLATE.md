...
> + short commit hash) — never a drag-select of the printed content, and
> never hand-typed: generated by `scripts/print_handoff_pointer.py`, which
> reads the branch and commit from git itself and refuses to print
> anything for a handoff not yet committed at HEAD, then re-verified by
> `scripts/check_handoff_pointer.py` before it is ever pasted — a
> hand-typed hash was proven fabricated once, see
> [`diagnosis/handoff-pointer-verification.md`](diagnosis/handoff-pointer-verification.md).
> The next agent runs the same check script as its first action on
> receiving a pointer, before trusting it. See
