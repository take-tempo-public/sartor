...
- [ ] **`_resolve_persona_*` transitional duplicate in `blueprints/generation.py`** — the
      generation seam (Sprint 8.3c, `refactor/app-blueprints-generation`) needs
      `_resolve_persona_template_path` / `_resolve_default_persona_template_path` for its
      `run_generation` / `run_generation_stream` / `download_edited` routes, but that pair belongs
      to the **templates/personas** seam (8.3e) and is **still called by the persona-preview
      routes that remain in `app.py`** — and a blueprint cannot import `app.py` (leaf-ward rule).
      Owner-decided bridge (2026-06-22): carry a **clearly-commented transitional copy** in
      `blueprints/generation.py` (the canonical copies stay in `app.py`), rather than promoting the
      pair to `web_infra` (deviates from the locked §3.4 seam-ownership table) or reordering seams.
      The two copies must stay in sync until the templates seam lands. _(discovered: v1.0.8 stream,
      2026-06-22, `refactor/app-blueprints-generation`.)_
      **→ Integrate at 8.3e (`refactor/app-blueprints-templates`):** when the templates seam moves
      `_resolve_persona_*` into `blueprints/templates.py`, **delete the generation copy** and import
      the pair from there (generation + the preview routes then share one canonical copy). This is
      the only tech debt 8.3c introduces; it is gone well before the v1.1.0 tag.
