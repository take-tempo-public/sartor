...
- [ ] **E2E walkthrough + R2-live verification (8.5 remainder)** — the gated test window's
      ...
      **Walkthrough residuals (post-Train-5)** row above for what those lanes didn't cover. The
...
- [ ] **Merge-suggestion similarity thresholds — first-pass, e2e-validated** — the
...
- [ ] **Eval baseline stale vs production model (Sonnet 5)** — after the
...
- [ ] **Walkthrough residuals (post-Train-5)** — six items surfaced by the owner-driven E2E
      walkthrough (2026-07-07/08) that Train 5's four repair lanes did not cover, tracked here
      as one aggregate row rather than re-opening the ledger six times: **persona-preview
      style-fidelity** (per-role spacing capture in `docx_to_persona_html.py`; owner
      **CONFIRMED** dates stay single-tab — no right-alignment work needed) ·
      **`preview_candidate_html` lazy-regen fallback gap** · **silent persona
      companion-generation failure** (needs surfacing — a companion `.html`/`.css` write can
      fail today with nothing telling the user) · **`/api/download-edited` identity-override
      wiring** (lane B follow-up note — verified against HEAD: `run_generation`/
      `run_generation_stream` (`blueprints/generation.py`) both pass
      `identity_override=_resolve_candidate_identity(context_set)` into
      `generator.generate_resume()`, but `download_edited` calls `generate_resume()` with no
      `identity_override` at all, since it works from raw edited content + username with no
      `context_set` in scope — so a re-download of hand-edited preview content bypasses the
      `fix/output-identity-and-dates` deterministic identity fix) · **document page-break
      preview/download parity** — accepted as a known limitation, not scheduled ·
      **Workday extractor research** (blocked on analyzing the owner's own NVIDIA application
      artifacts — the real choke sample; see nursery item #7). Drains via a single
      `fix/persona-fidelity-and-residuals` lane before the code freeze.
      _(discovered: v1.0.8/Train-5 stream, 2026-07-08, merge-train capture; open count 8 → 9.)_
