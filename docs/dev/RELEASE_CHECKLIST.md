...
_Open count: 10 — at the top of the ~8–10 reduction-sprint threshold, but the two newest items self-clear imminently (the `_resolve_*` duplicate at 8.3e, the `list_resumes` hardening at 8.3f), so no standalone reduction sprint is needed yet; re-evaluate at 8.3g if no items have drained. **Triaged 2026-06-20 (7.9 ledger capture):** each item below carries a **→ integrate at 8.x** target mapping it to an already-scheduled v1.0.8 sprint, so the ledger drains without new standalone branches. **`chore/ledger-reduction` (8.0) ran 2026-06-21** and cleared the **CONTRIBUTING-drift + pytest-socket `UserWarning`** pair (both now under Resolved), dropping the open count 10→8. **`refactor/app-blueprints-generation` (8.3c) added one item** — the `_resolve_persona_*` transitional duplicate (→ clears at 8.3e) — bumping 8→9. **`refactor/app-blueprints-corpus` (8.3d) added one item** — the pre-existing `list_resumes` raw-username observation surfaced while moving the route (owner chose to track it 2026-06-22, → 8.3f); its shared `_tag_list`/`_skill_to_dict` used the legal `app.py → blueprint` import (no transitional duplicate) — bumping 9→10. **`refactor/app-blueprints-templates` (8.3e) ran 2026-06-22** — RESOLVED the `_resolve_persona_*` duplicate (now under Resolved) and ADDED the `_load_application_owned` transitional duplicate (→ clears at 8.3f); net open count unchanged at **10**. Net drain from here: 8.5 clears the S3-eval + grounding-metric pair · 8.7 clears portable-core + link-checker + flaky-UX · 8.3f clears the `list_resumes` raw-username hardening **and** the `_load_application_owned` duplicate · 8.3 clears help-opener · 8.6a clears assistant doc-coverage · the citation viewer stays deferred → by v1.1.0 the ledger is ~2 items._
...
- [ ] **`_load_application_owned` transitional duplicate in `blueprints/templates.py`** — the
      templates seam (Sprint 8.3e, `refactor/app-blueprints-templates`) needs `_load_application_owned`
      (the `(app_row, candidate)` ownership resolver) for its `preview_application_html` /
      `preview_cover_letter_html` routes, but that helper belongs to the **applications** seam (8.3f)
      and is **still called by the ~10 applications routes that remain in `app.py`** — and a blueprint
      cannot import `app.py` (leaf-ward rule). Owner-decided bridge (2026-06-22, mirroring the 8.3c
      `_resolve_*` decision): carry a **clearly-commented transitional copy** in `blueprints/templates.py`
      (the canonical copy stays in `app.py`), rather than promoting it to `web_infra` (would rewrite all
      ~10 app.py call sites to thread `configs_dir`, expanding into 8.3f's seam against the one-seam
      scope guard) or reordering seams. The one behavior port from the app.py copy: it calls
      `_safe_username(candidate.username, configs_dir=current_app.config["CONFIGS_DIR"]) `(the `web_infra`
      signature). The two copies must stay in sync until the applications seam lands. _(discovered:
      v1.0.8 stream, 2026-06-22, `refactor/app-blueprints-templates`.)_
      **→ Integrate at 8.3f (`refactor/app-blueprints-applications`):** when the applications seam moves
      `_load_application_owned` into `blueprints/applications`, **delete the templates copy** and import
      it from there (templates + the remaining app.py callers then share one canonical copy). The only
      tech debt 8.3e introduces; gone well before the v1.1.0 tag.

- [ ] **`list_resumes` builds its path from a raw (un-`_safe_username`'d) username** —
      `list_resumes` (`GET /api/users/<username>/resumes`, moved to
      `blueprints/corpus/curation.py` in 8.3d) builds
      `current_app.config["RESUMES_DIR"] / username` from the **raw** route `username` and lists
      that directory, *without* the `_safe_username` sanitize+existence guard its sibling corpus
      routes use. Pre-existing (predates the epic); **preserved byte-identically** under the 8.3d
      pure-refactor rule (not introduced by the move). Low severity — likely not URL-exploitable
      (Flask's `<username>` string converter rejects `/`, and `..` path segments collapse before
      routing) — but it is the one corpus FS-touching route reaching the filesystem without
      `_safe_username`, surfaced while moving it. _(discovered: v1.0.8 stream, 2026-06-22,
      `refactor/app-blueprints-corpus`; owner chose to track 2026-06-22.)_
      **→ Integrate at 8.3f (`refactor/app-blueprints-applications`)** (or the users/config seam,
      whichever settles the user-scoped routes): add
      `_safe_username(username, configs_dir=current_app.config["CONFIGS_DIR"])` and return the empty
      list / 400 on failure, matching the sibling routes — a small behavior tightening (it changes
      which usernames the route accepts), so it needs its own sign-off rather than riding silently.
