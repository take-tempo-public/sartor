...
1. **PX-39 real-corpus baseline (step 12)** — do first/soon: unblocked *now*, cheap (well under $1), and needs exactly the kind of session (`.api_key` present, not a worktree) this scoping pass ran in. Retires the defunct latency rows before they seed another false alarm.
