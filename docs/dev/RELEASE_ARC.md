...
11b. **PX-44 follow-on (owner decision, not yet scheduled):** the 46-file fixture-scoping rollout, now evidence-backed by the `test/fixture-scoping` pilot (item 11 above) instead of a projection — removes ~99% of the `init_db` slice (0.138s→0.0015s median per converted test) but leaves `create_app` (0.108s, untouched) as the new dominant per-test cost, so the realistic ceiling is the `init_db` slice's share of the fast lane, not the whole ~0.12s/test average. DX-only, not user-facing. Open owner decision: worth doing pre-tag, or defer post-public? Full detail: `docs/dev/perf/TEST_SUITE_PERFORMANCE.md`.
...
4. **PX-44 46-file rollout (step 11b)** — DX-only; owner decides pre-tag vs. post-public.
