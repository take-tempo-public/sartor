...
16. **`[HUMAN]`-only, gates the tag itself:** GitHub repo public flip, PyPI Trusted Publisher config, GHCR package visibility, CodeQL wired as a required status check, branch protection required-checks update. All are GitHub-settings/owner toggles — no repo file changes. Concretely, verified 2026-07-21: (a) rename repo to `take-tempo-public/sartor` + flip public (blocks the other three); (b) PyPI Trusted Publisher — project `sartor`, environment `pypi` (already referenced in `release.yml:89,98`); (c) GHCR package visibility → public (`ghcr.io/take-tempo-public/sartor`, `docker.yml:53`; optional cleanup: delete the unused `sartor-python-models` GHCR package); (d) add `Analyze (python)` + `Analyze (javascript-typescript)` (CodeQL) to `main`'s required status checks (Settings → Branches) — resolver work is done (0 open `py/path-injection` alerts), only the required-check wiring is outstanding; (e) `enforce_admins` on `main` branch protection — currently `false`, so an admin can bypass the 6 required checks; the flip is a standing open owner decision (`RELEASE_CHECKLIST.md`).
