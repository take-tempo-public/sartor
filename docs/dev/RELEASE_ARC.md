...
| PX-44 | Fixture-scoping **refactor** (module-scope read-mostly test fixtures) | **Piloted 2026-07-21** (`test/fixture-scoping`, step 11 above): doc/CONTRIBUTING fix landed earlier; the refactor itself piloted a migrated-template-DB-copy mechanism (not module-scoping — no safe target existed) on 2 files, proven isolation-safe, 46-file rollout deliberately deferred as a follow-on. |
...
Most landed (#1/#3/#7/#8/#9/#10/#11/#13/#15/#17 all confirmed). Two remained open as of the audit that wrote this section — both are now **DONE**:
- **The run-cancel/abort endpoint** — the owner explicitly opted **in** to a real abort path, not just a click-lock. Landed in step 9 (`feat/diagnostics-run-cancel`).
- **The #2/4/5/6/16 instructional content cluster** — the small draft down-payment (a handful of tooltips) is now superseded by the full field-level `_DASH_HELP` authoring pass. Landed in step 10 (`docs/diagnostics-content-cluster`).

All 17 diagnostics round-2 items are now closed; the v1.1.0 close-out arc's diagnostics-DX residue is fully resolved.
...
**Steps 12/13/15/16/17 (2026-07-28) — the not-yet-DONE remainder — MIGRATED to
[`docs/dev/work/BOARD.md`](work/BOARD.md)** as work items 6 (PX-39), 7 (PX-46),
9 (visual assets), 3 ([HUMAN] GitHub toggles), and 10 (release cut,
`depends_on` the other four), respectively. DONE steps below are unaffected
and stay as the historical record of how each landed.
...
8. `chore/hook-dispatcher` (PX-37 + kit-adoption commitment 3) — consolidate the 5 hooks, re-home to root `skills/`/`hooks/`. Touches the enforcement surface directly — single session, verify each hook still fires correctly before merging. **Coordinate with step 5** — both touch the enforcement core; step 5 lands first. **DONE** (`chore/hook-dispatcher`): collapsed the 5 Edit|Write PreToolUse guards (`require-feature-branch`, `require-evidence-before-fix`, `block-secrets`, `validate-context`, `route-security-lint`) into one `hooks/edit-write-dispatcher.sh` entry (`scripts/enforcement/adapters/claude_dispatcher.py`), replacing 5 separate per-edit process spawns with one — no short-circuit, every blocked guard's messages still aggregate, matching the pre-existing parallel-hook behavior (regression test: `tests/test_enforcement_core.py::TestEditWriteDispatcher::test_aggregates_two_simultaneous_blocks_no_short_circuit`). Re-homed all 13 hook scripts from `.claude-plugin/hooks/` to root `hooks/`. **Root `skills/` scoped down to an empty scaffold this session** (owner decision) — the `context-structure-review` skill import itself has no source content in this repo and needs a `CLAUDE.local.md`-recorded kit path first; filed as its own open follow-on (see `skills/README.md` + `RELEASE_CHECKLIST.md`'s kit-adoption ledger row). Updated the 4 test files whose 1-script-per-guard assumptions this broke (`test_governance_hooks_gate.py`'s `BLOCKER_HOOKS`/`BLOCKER_RULE_NAMES` split, `test_enforcement_core.py`'s `GUARD_FILES` repoint, `test_evidence_gate.py`, `test_plan_approval_scoping.py`), all green. Manually fired block + allow through the real `hooks/edit-write-dispatcher.sh` for all 5 guards in a disposable throwaway repo (byte-correct JSON via `json.dumps`, per this repo's own convention) — all 10 cases confirmed correct, including a live no-short-circuit observation (two guards tripping on one payload, both messages surfaced). Also live-confirmed `block-merge-to-main.sh` and `block-secrets.sh` firing from their new `hooks/` path when they correctly blocked two of this session's own real tool calls. Full gate green: `ruff check .` + `ruff format --check .` (314 files) + `mypy .` (329 files) + `pytest` (2035 passed/1 skipped non-UX + 121 passed UX) — the gate run itself caught 10 broken cross-doc markdown links the hooks re-home left behind (`test_doc_links.py`), fixed in a follow-up commit.
8b. `fix/context-write-lost-update-gap` — **unplanned insertion, not in the original sequence** (found while researching step 9's `threaded=True` question; numbering left as-is rather than renumbering steps 9+ and their cross-references). **DONE**: 5 more `context_*.json` read-modify-write routes (`blueprints/analysis.py`'s `/api/clarify`, `/api/answer-clarifications`, `/api/iterate-clarify`; `blueprints/generation.py`'s `/api/save-edits`, `/api/generate-cover-letter`) shared the exact unprotected shape the `fix/compose-frozen-composition` lost-update fix closed at 12 sites in `blueprints/applications.py` only — never audited against it. Dynamically reproduced (falsification test fails on HEAD, `docs/dev/diagnosis/context-write-lost-update-gap.md`) before fixing; converted all 5 to `hardening.context_transaction`, matching the established pattern. Also corrected a related, dated documentation error (`hardening.py`'s `context_transaction` docstring + the original diagnosis dossier both falsely claimed the production server runs threaded — it never has) and filed a new carry-forward ledger item on a compliance-witness scope gap this exposed. Full gate green: ruff + mypy + 2038 non-UX tests (1 pre-existing skip) + 78 UX tests (one unrelated pre-existing scroll-flake confirmed via isolated re-run, not a regression). 5 commits.
9. `feat/diagnostics-run-cancel` — the real abort endpoint (owner opted in). Touches run-lifecycle/threading; read `docs/dev/reviews/2026-07-diagnostics-round2-findings.md`'s RUN-LIFECYCLE note first. **DONE** (`feat/diagnostics-run-cancel`): disconnect-as-cancel, not a literal second route — `app.run()` stays single-threaded (that flag is still its own deliberately-deferred governance decision), so a real second `POST /cancel` couldn't be serviced while the original SSE connection is open; the cancel signal travels over that SAME connection instead. All 4 SSE routes in `blueprints/diagnostics.py` (`annotation_score_grounding`, `annotation_bootstrap_stream`, `eval_run_stream`, `tune_run_stream`) switched their blocking `events.get()` to a 5s-timeout poll that yields a heartbeat SSE comment on `queue.Empty`; the whole generator body is wrapped in `try/except GeneratorExit`, which fires when a real client disconnect (or the new frontend Cancel button's `AbortController.abort()`) causes the next write to fail and Werkzeug tears the generator down — the handler sets a per-request `threading.Event` the worker thread polls before its next paid/CPU call. `run_suite` (`evals/runner.py`), `run_pipeline_over_jd_texts` (`evals/bootstrap.py`), and `run_grounding_signals` (`evals/grounding_signals.py`, zero paid calls — checkpoint shortens wall-clock only, not billing) each gained an optional `cancel_check: Callable[[], bool] | None = None` param mirroring the existing `progress` pattern (additive, every existing caller unaffected); `run_suite` checkpoints at 6 points (top of fixture loop, before clarify, before generate/assemble, before the distinctiveness judge, top of the rubric-grading loop) — broader than the handoff's literal analyze/clarify/generate list, since the findings doc's own wording is "before its next paid Anthropic call," not "before its next fixture." `tune_run_stream`'s worker skips the candidate pass entirely if cancellation lands during baseline. Frontend: `window.sartorEval.stream()` gained `AbortController` support + a shared `wireCancel`/`hideCancel` helper; 4 new Cancel buttons (Quality/Tuning/Bootstrap/Annotate), each hidden until a run starts, showing an optimistic "Cancelling…" (no server confirmation can reach the client post-disconnect, per the already-accepted UX limitation). The bootstrap and grounding-score routes' hand-rolled `fetch`+`getReader` pumps were folded into `window.sartorEval.stream()` as part of this branch (Cancel needed identical wiring in all 4 places, so implementing it once removed ~70 duplicated lines rather than growing 2 more hand-rolled `AbortController`s) — `test_20260709_diagnostics_run_lock.py`'s stale "NOT window.sartorEval.stream" docstring claim corrected in the same branch that invalidated it. New tests: 3 `run_suite` cancel unit tests, 2 `run_pipeline_over_jd_texts`, 2 `run_grounding_signals`, 4 route-level falsification tests (Flask test client `.close()` on the undrained WSGI iterator to deliver a real `GeneratorExit`, proving `cancel_check()` reads `True` after), 2 Playwright browser tests (real `AbortController.abort()` via a held route interceptor). Full gate green: `ruff check .` + `ruff format --check .` + `mypy .` (331 files) + `pytest` — 2050 passed/1 skipped non-UX (chunked into 6 foreground batches after background bash runs were repeatedly silently killed ~5-7 min in, matching the known background-kill-ceiling pattern) + 123 passed UX (a11y+flows together, regression split into 3 chunks).
10. `docs/diagnostics-content-cluster` — the remaining #2/4/5/6/16 field-level `_DASH_HELP` authoring pass. Content-only. **DONE** (`docs/diagnostics-content-cluster`): authored 9 new field-level `_DASH_HELP` entries (`dashSuiteField`, `dashSubsetField`, `dashGroundingSignals` — each shared verbatim across the 2-3 tabs where that control repeats — plus `dashTuneConstant`, `dashTuneCandidate`, `dashTuneSeed`, `dashBootstrapRun`, `dashVerdicts`, `dashAnnScore`), matching the main app's `_HELP_REGISTRY` voice (2nd person, 2-5 sentences, plain-text bodies — the shared `help-modal.js` opener sets `.textContent`, so no HTML/links render; deep-doc refs stay literal paths). Added 13 static `.help-info` icon instances next to their controls across Quality/Tuning/Annotate (Pipeline/Groundedness stayed tab-level only — read-only tiles, nothing granular to explain). Icons are siblings of their `<label>`/button, never nested inside one — a `<button>` inside a `<label>` risks the label's default click-forwarding double-firing the wrapped control, so every icon sits in the existing flex row (`.filters`, `.tune-editor-head`, `.annotate-actions`) instead. Broadened the one click-handler selector (`dashboard.html` wiring IIFE) from `.dash-pane-intro .help-info` to `.help-info[data-help]` so field-level icons (which live outside the intro paragraph) wire up through the same handler as the 5 pre-existing tab-level icons; field icons carry no auto-open (`_maybeFireDashHelp` untouched), so they stay click-only like the main app's field entries. Also rewrote `dashTuning`'s prompt-tuning-loop explainer in plainer language per finding #6 (dropped the ALL-CAPS emphasis, shorter sentences). Verified live: full quality gate green (ruff + ruff format + mypy 331 files + `pytest -m "not ux"` 2049 passed/1 pre-existing skip + `pytest -m ux` 123 passed, matching the prior session's exact UX count) plus a real Chromium/Playwright click-through confirming all 9 new entries open with correct title/body and zero console errors. Checked a `≈$0.30` "full"-subset cost figure already in `dashQuality`'s body against `evals/README.md:936` + `TUNING_LOG.md:1529` before touching nearby text — it is accurate (matches the current committed cost), not a bug. That check surfaced a real, separate staleness in `AGENTS.md:193`: its `~$1.50` figure for the full synthetic suite was the pre-Sonnet-5 historical cost, superseded by the same cache/cost work `TUNING_LOG.md:1529` documents (`$0.4068` measured). One-line, unambiguous, verified-not-guessed — fixed directly in the same commit rather than opening a new carry-forward ledger row for something this small, given the ledger's own reduction-sprint pressure (see `RELEASE_CHECKLIST.md`).
11. **Owner decision point:** `test/fixture-scoping` (PX-44) — the original prescription itself flagged this as touching test isolation mid-epic risk. Decide whether the refactor is worth doing pre-tag or should defer post-public; it is DX-only, not user-facing. **Owner decided pre-tag, pilot-only scope (2026-07-21).** **DONE, pilot** (`test/fixture-scoping`): mechanism re-evaluated against the exact code before piloting — the perf doc's own SAVEPOINT recommendation turned out to be structurally wrong (routes open their own `get_session()` connection and commit mid-request; a test-held `begin_nested()` can't wrap that without re-plumbing production session wiring), and module-scoping a read-only file has no safe target (all 46 `create_app`-using files seed the DB per test, even the GET-only ones). Piloted a third mechanism instead — a session-scoped migrated-template DB, copied per test (`tests/conftest.py::_migrated_template_db`) — on 2 files (`test_corpus_duplicates_route.py`, `test_clarifications_list.py`), zero test-body changes. Two traps closed: `init_db()` memoizes on a path set, not DB state (copies must be pre-registered or the first route re-migrates them); every connection runs `journal_mode=WAL`, so a naive file copy can be schema-complete but seed-empty (closed via `wal_checkpoint(TRUNCATE)` before the first copy). Isolation proven under 3 explicit test orderings (forward/reversed/scrambled); full fast lane identical on both sides of the diff (2053 passed/1 skipped). Decomposed timing: removes ~99% of the `init_db` slice (0.138s→0.0015s median) but leaves `create_app` (0.108s, untouched) as the new dominant per-test cost. **46-file rollout deliberately NOT done here** (owner scope decision: pilot only) — filed as the natural PX-44 follow-on with this pilot's numbers as its evidence, not projection. Full detail: `docs/dev/perf/TEST_SUITE_PERFORMANCE.md`.
11b. **PX-44 follow-on.** **Owner-directed 2026-07-27** (picked as the next branch from the carry-forward ledger's 3-remaining-of-20 efficiency-review row). **DONE** (`test/fixture-scoping-rollout`): the 46-file fixture-scoping rollout, evidence-backed by the `test/fixture-scoping` pilot (item 11 above). Re-derived the target list fresh (5-day-stale pilot counts, not trusted): 44 files matched the pilot's mechanical fixture shape; factored the pilot's hand-duplicated copy/monkeypatch/register block into one shared `_fresh_migrated_db` helper in `tests/conftest.py` rather than duplicating it 44 more times. One new exclusion found mid-rollout (`test_bundled_templates.py::TestSeedMigration` directly tests `init_db()`'s own migration behavior — same rationale as the already-excluded `test_db_session.py`). `test_context_write_races.py::races_app` (concurrent-request races against the same DB file) got a dedicated before/after flake-rate check — 15 runs each side, 0/30 failures both ways — before converting. Batched verification (11/13/9/11-file groups): every batch's pass count identical forward, reversed, and cross-batch-interleaved. Full fast-lane chunked run: 2055 passed/1 skipped/1 failed — the 1 failure is this branch's own file-touch volume (46 files) crossing the wiki-freshness gate's 75-file merge-block threshold, not a functional regression; resolved at close-out via `/wiki-self-update`, not a rollout defect. This session's own timing microbenchmark: `init_db` 84.4ms → `copy2` 1.0ms median (98.8% reduction), consistent with the pilot's ~99% figure. Full detail: `docs/dev/perf/TEST_SUITE_PERFORMANCE.md` "Rollout result" section.
12. **UNBLOCKED (verified 2026-07-21) — `.api_key` present, this is not an isolated worktree:** `perf/real-corpus-baseline` (PX-39) — the actual Sonnet-5 real-corpus measurement. The prior blocker is cleared. **Data source updated 2026-07-21:** the owner has a separate E2E clone holding genuinely real user data, richer than the current `testuser` fixture — export it via `scripts/export_corpus_seed.py` into a gitignored `evals/fixtures/real/<user>/seed.json` (never committed, per AGENTS.md's real-data rule) before running the baseline, so the measurement reflects real usage rather than a thin test fixture. Requirement: run a handful of real analyze→clarify→generate cycles under Sonnet-5 against that seed (`evals/runner.py --suite real --seed evals/fixtures/real/<user>/seed.json`) → aggregate p50/p95 + cost/wall-clock from `logs/llm_calls.jsonl` via `scripts/perf_baseline.py`, **segmented by model and by frozen-composition vs fallback/legacy `generate` path** → record into `docs/dev/perf/PERFORMANCE_HISTORY.md` (fill the Era-3 real-corpus row, close its "Open item" section, retire the DEFUNCT Era-2 69.7s p50 / 84.6s p95 rows as no longer the current reference). Cost well under $1 (eval-smoke precedent ~$0.35-0.40 for a full synthetic smoke).
    **RESOLVED 2026-07-28, differently than prescribed above** (work item 6):
    the method above cannot run as written — `evals/runner.py` hardcodes an
    `eval:` prefix on `username` at every call site (including inside
    `--suite real`) and uses that prefix as its own cost-attribution key
    (`evals/runner.py:385`), so a harness run can never produce the
    non-`eval:*` traffic this step asks for; `--suite real` is additionally
    non-functional today (no fixtures exist, work item 16). Closed instead
    using zero-spend real historical telemetry (owner's ordinary app usage,
    2026-07-06 → 2026-07-28) copied into `logs/llm_calls.jsonl`. That measurement
    also found the pipeline-shape change from `fix/compose-frozen-composition`
    (merged 2026-07-06, one day into this era) retired `generate()` from the
    dominant real-corpus path, so the Era-3 real-corpus row this step asks to
    fill has no subject anymore — see the new **Era 4** section in
    `PERFORMANCE_HISTORY.md` for the actual baseline and metric definition.
13. **Owner-gated, do not schedule without sign-off:** `chore/memory-consolidation` (PX-46) — present the keep/consolidate/delete list first; act only after explicit approval. Prescription (`docs/dev/reviews/2026-07-efficiency/prescriptions.md:53`): **selective**, not wholesale — fold the ≥3 unique-recipe memories (seam-move mechanics; mypy-strict + ruff-D verify commands) into a durable `docs/dev/` reference, delete only genuinely redundant completion logs, shrink the freed `MEMORY.md` index lines. Judges: **irreversible if botched — no urgency justifies the busy window.** ~153 memory files exist as of 2026-07-21 (re-verify the count fresh before acting — it grows session to session).
14. Check accumulated CI runs on `main` post-scroll-fix-merge → resolve the `--reruns 2` retry-policy decision (RELEASE_CHECKLIST carry-forward item 1) and confirm the scroll-flake CI leg. Read-only investigation; may not need its own branch. **DONE** (`docs/scroll-flake-ci-data-rerun-policy`): pulled all 12 CI job logs on `main` since the scroll-fix's own landing run — 0/12 red, but 5/12 (~42%) fired a rerun across 5 distinct settle/restore-family tests (one run 2/3-failed on a single test). Owner decided **(a) keep `--reruns 2` + add a rerun-rate alarm** (dropping reruns would flash `main` red ~42% of the time from load, not regressions; leaving it silent repeats the exact blind spot that hid the original bug). The alarm itself is a code change, not built here — filed as an open follow-on under RELEASE_CHECKLIST ledger item 1. Also confirms RELEASE_CHECKLIST ledger item 4's previously-unmet "CI leg of the acceptance bar" for the mode-C scroll flake. Full data in `RELEASE_CHECKLIST.md`. **Follow-on BUILT (2026-07-21, `feat/rerun-rate-alarm`):** the alarm itself — `$GITHUB_STEP_SUMMARY` table + `::warning::` annotation per absorbed rerun, report-only (never fails the build) — landed; RELEASE_CHECKLIST ledger item 1 moved Open → Resolved. Full detail there and in that branch's `CHANGELOG.md` entry.
15. `release/visual-assets` + a fresh-clone re-verify (Phase 5 tag criteria: `docs/screenshots/` already has 10 images — confirm still current; fresh-clone < 5 min; doc links resolve). **Current state (verified 2026-07-21): the 10 PNGs are ~7.5 weeks stale** (last committed 2026-05-28, predating the diagnostics-console redesign and everything since). Capture is a working, de-bugged Playwright run (`scripts/capture_screenshots.py`, ~$0.27 Anthropic spend, ~8 min, requires `python app.py` running locally first) — the 3 staleness bugs found in `fix/capture-screenshots-welcome-modal` are fixed. **Gap to close in this sprint:** `readme_hero_wizard-step1-filled.png` was captured for the README hero but was never wired into `README.md` — README currently has zero screenshot references. **Stays an open owner decision, not in this sprint's scope:** `scripts/capture_screenshots.py` still has zero periodic/CI coverage, which is exactly what let the 3 staleness bugs accumulate silently for ~7 weeks (carry-forward ledger item).
16. **`[HUMAN]`-only, gates the tag itself:** GitHub repo public flip, PyPI Trusted Publisher config, GHCR package visibility, branch protection required-checks update. All are GitHub-settings/owner toggles — no repo file changes. Concretely, verified 2026-07-21, corrected 2026-07-22 (`chore/dependabot-ci-infra`): (a) rename repo to `take-tempo-public/sartor` + flip public (blocks the other two); (b) PyPI Trusted Publisher — project `sartor`, environment `pypi` (already referenced in `release.yml:89,98`); (c) GHCR package visibility → public (`ghcr.io/take-tempo-public/sartor`, `docker.yml:53`; optional cleanup: delete the unused `sartor-python-models` GHCR package); ~~(d) add `Analyze (python)` + `Analyze (javascript-typescript)` (CodeQL) to `main`'s required status checks~~ **already done** — verified directly against `main`'s branch protection via the GitHub API on 2026-07-22: both checks are already required. This item and its `RELEASE_CHECKLIST.md` twin both stated the wiring was still outstanding; both were stale, not this session's finding of a new gap — see `chore/dependabot-ci-infra`'s CHANGELOG entry; (e) `enforce_admins` on `main` branch protection — currently `false`, so an admin can bypass the 6 required checks; the flip is a standing open owner decision (`RELEASE_CHECKLIST.md`).
17. `chore/release-v1.1.0` — CHANGELOG cut to `[1.1.0]`, version bump, tag — executed on the owner's go. Exact mechanics verified 2026-07-21: bump `pyproject.toml:7` `1.0.9` → `1.1.0`; rename `CHANGELOG.md`'s `## [Unreleased]` (currently line 14) to `## [1.1.0] — <date>`, **keeping the `### Fixed vulnerabilities` block inside it** (this is what activates and satisfies the D-7.4 disclosure gate in `tests/test_release_versioning_gate.py`, currently skipped because the newest released section is `[1.0.9]`, pre-D-7); optionally re-add an empty `[Unreleased]` above it. Cut + push tag `v1.1.0` — `release.yml`'s tag-match step validates it against the bumped `pyproject.toml` version via `scripts/release_version.py`.

Steps 11/11b/12/13/16 are explicitly owner-gated or condition-gated, not agent-schedulable in sequence — the closing agent on each preceding branch should surface them rather than attempt them blind. (Renumbered 2026-07-19 when steps 5 and 6 were inserted; the old 5-15 are now 7-17.)

### Recommended endgame order (scoped 2026-07-21 — owner's call to accept or reorder)

1. ~~**PX-39 real-corpus baseline (step 12)**~~ — **DONE** 2026-07-28, `docs/pipeline-truth-and-era4-baseline` (work item 6). See that step's resolution note above and `PERFORMANCE_HISTORY.md`'s Era 4 section.
2. **`release/visual-assets` refresh (step 15)** — mechanical, tag-required; regenerate the 10 stale screenshots and wire the README hero into `README.md` in the same sprint.
3. **PX-46 memory consolidation (step 13)** — owner-gated; run whenever the keep/consolidate/delete list is authorized. Not strictly tag-blocking but is a v1.1.0-gate item.
4. ~~**PX-44 46-file rollout (step 11b)**~~ — **DONE** 2026-07-27, `test/fixture-scoping-rollout`.
5. **`[HUMAN]` GitHub toggles (step 16)** — around the tag; owner-only, no agent role.
6. **Release cut (step 17)** — last: version bump + CHANGELOG `[1.1.0]` + tag, on the owner's go.
