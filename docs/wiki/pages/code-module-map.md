...
| Module | Job | Anchor |
|---|---|---|
| [`scripts/gate.py`](../../../scripts/gate.py) | Unified quality gate (PX-55): the single definition of "gate green" that runs locally and in CI. Runs, in order: `ruff check .` + `ruff format --check .` + `mypy .` + `pytest -m "not ux" -n auto` (non-UX tier parallelized via pytest-xdist) + `pytest -m ux` (UX tier deliberately serial) + `work_items check`, returning the first failing step's exit code. Consolidates what was scattered across `.github/workflows/ci.yml`'s `quality` job, `AGENTS.md`, and `CONTRIBUTING.md` before this module existed. | [`scripts/gate.py:_STEPS`](../../../scripts/gate.py), [`scripts/gate.py:_run_step`](../../../scripts/gate.py) |
| [`scripts/work_items.py`](../../../scripts/work_items.py) | Work-item backlog validator and board generator (vendored from `spolia`); schema authoritative at [`docs/dev/work/SCHEMA.md`](../../../docs/dev/work/SCHEMA.md). Two subcommands: `check` validates every item file + confirms board freshness; `board --write` regenerates `docs/dev/work/BOARD.md`. Wired into `scripts/gate.py` as the final step. Stdlib-only. | [`scripts/work_items.py:check`](../../../scripts/work_items.py), [`scripts/work_items.py:render_board`](../../../scripts/work_items.py) |
