...
- **`last_generated_resume` / `last_generated_cover_letter`** ← the exact text just produced for this iteration. For the résumé, that is the LLM's output on the legacy `generate()` path, or (Generation-experience re-architecture Phase 4) the deterministic `json_resume_to_markdown` view of the frozen `approved_composition` on the zero-LLM frozen-composition path — `save_iteration_context` itself doesn't care which; it snapshots whatever `resume_content` the caller passed `[synthesis]`. The cover letter, when requested, is always an LLM call in either branch. The [`ContextSet`](../../../hardening.py) header calls these the *"frozen-at-generation snapshot"* the frontend diffs the live preview against to detect user edits.
...
[`blueprints/generation.py:run_generation`](../../../blueprints/generation.py) (the `/api/generate` handler) and its SSE twin [`blueprints/generation.py:run_generation_stream`](../../../blueprints/generation.py) both invoke `save_iteration_context` with `parent_path=str(cp)` (the input context), `action="generate"`, and a `summary` it composes from whether refinement notes or an edited baseline were present — unconditionally, regardless of whether the résumé step took the LLM `generate()` branch or the Phase 4 frozen-composition deterministic-assemble branch (see [[pipeline-stages]] Step 5) `[synthesis]`. The route then returns the **new** child path as `context_path`, plus `iteration` and `parent_context_path`, in its JSON (and the stream's `done` event). The handler docstring is explicit that *"the frontend must use \[the new context_path] for any subsequent calls… so the iteration chain is preserved"* ([`blueprints/generation.py:run_generation`](../../../blueprints/generation.py)).
...
- [[pipeline-stages]] — where `/api/generate` sits in the analyze → generate → iterate flow, including the Phase 4 frozen-composition branch.
- [[corpus-to-output-reach]] — how the résumé text this chain snapshots is assembled (LLM or deterministic).
- [[code-module-map]] — `hardening.py` (deterministic lifecycle) vs. `blueprints/` (routes).
