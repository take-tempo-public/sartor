...
| Parallel-session isolation (W-1) | **worktree-local hooks: DONE; written governance: still open** | the "make the plan hooks worktree-scoped" half **shipped** on `fix/plan-approval-hook-scope` — `check-plan-approved.sh`/`mark-plan-approved.sh`/`cleanup-plan-on-merge.sh` now key their marker/pointer state off `CLAUDE_PROJECT_DIR`, so a concurrent session in a different project/worktree can no longer false-block or wipe this one's approved plan (F-gov-02). **Correction:** "charter W-1" was cited here (and elsewhere) as if it already existed as prose governance — it does not; `charter.md` has no `W-1` clause (only `C-0`…`C-9`, `D-1`…`D-7`). That citation gap is filed as its own `RELEASE_CHECKLIST.md` Carry-forward item for a future, owner-directed amendment ceremony (F-gov-03 stays open until that clause is actually written) |
