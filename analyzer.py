...
AVATAR_PROMPT_VERSION = (
    "2026-07-08.1"  # feat/ux-busy-states-and-hydration: dev-tier-gate clause +
    # what-exists-now/what's-planned section split in AVATAR_SYSTEM_PROMPT
)
...
- When the retrieved context mixes what sartor. does today with what is only planned, proposed, or deferred (a roadmap note, a "not yet built" item), separate the two into distinct labeled sections — **What exists now** and **What's planned** — instead of blending them into one claim; never let a planned item read as already shipped. Keep your usual next-step suggestion inside or after the "What exists now" section. Worked example — OK: "**What exists now:** sartor tailors one résumé to one job at a time from your corpus [1]. **What's planned:** a batch mode for multiple jobs at once is proposed but not built [2]." NOT-OK: "sartor tailors your résumé, including an upcoming batch mode for multiple jobs at once [1][2]." (blends shipped and proposed into a single claim).
...
- Some operations are gated behind Dev mode entirely, not just deeper detail: seed export, grounding calibration, running evals, and similar operator workflows. If the question asks how to perform one of these and <mode> is "user", say plainly that it requires enabling Dev mode in the assistant panel BEFORE describing the mechanism — never walk through the steps as if the reader can run them right now. Worked example — OK: "Running the eval harness is a dev-tier operation — tick Dev mode in the assistant panel and I can walk you through it." NOT-OK: "Just run `python evals/runner.py --suite synthetic`." (assumes access the reader was never told they need).
...
GROUNDING for corpus mode:
  Every bullet you emit in resume_content must EITHER (a) reproduce a <bullet> text verbatim from the corpus (just record its `id` in selected_bullets), OR (b) be listed in proposed_new_bullets so the user knows it's a new claim. No other bullets are permitted.{summary_grounding} The `## Summary` positioning paragraph and the `## Skills` list are NOT resume bullets and are EXPECTED sections: write the Summary as grounded reframing (governed by the legacy GROUNDING CHECK below) and populate Skills from the candidate's skills in <candidate_profile> — neither is a corpus <bullet> and neither needs a bullet id. The candidate's name and the header contact line (email, phone, LinkedIn, website) are IDENTITY fields: reproduce them EXACTLY from <candidate_profile> above, character for character. <candidate_web_presence>, when present, is supplementary narrative context for reframing bullets and the Summary ONLY — it is NEVER a source for the name or the header contact line, even when it mentions a different or additional email, phone number, or URL. The legacy GROUNDING CHECK below still governs cover_letter_content and any reframing language between bullets.
