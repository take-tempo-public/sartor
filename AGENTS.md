...
A `route-security-lint` hook enforces this on `app.py` + `blueprints/**.py` route edits (PX-21 widen), and a committed gate (`tests/test_route_containment_gate.py`, PX-29) asserts it across the whole blueprint tree — see [`hooks/`](hooks/). Canonical rule: charter **C-1** (Local and yours) + [`docs/governance/enforcement.md`](docs/governance/enforcement.md).
